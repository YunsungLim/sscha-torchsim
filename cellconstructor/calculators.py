...
# FOR BATCH CALCULATIONS (using TORCH-SIM)
import torch
...
def get_results_batch(calculator, structures, get_stress = True, recompute=True):
    """
    Accepts an ASE calculator (within Torch-sim)
    and computes all the implemented properties (energy, forces and stress tensor).
    ...
    ----------
        calculator : ase.calculators.calculator.Calculator or Calculator
            The calculator to use for the calculation
        structures : list of CC.Structure.Structure
            The structures to compute
        get_stress : bool
            If true, the stress tensor is computed
        recompute : bool
            If true, the calculation is recomputed.
    ...
    ase_atoms = [structure.get_ase_atoms() for structure in structures]
    init_state = ts.initialize_state(
        ase_atoms,
        device=device,
        dtype=dtype,
    )

    calc_result = ts.static(
        system=init_state,
        model=calculator,
        autobatcher=True,
    )
    ...
    for d in calc_result:
        energy = d['potential_energy'].detach().cpu()[0]
        forces = d['forces'].detach().cpu().numpy()
        stress = d['stress'].detach().cpu().numpy()
