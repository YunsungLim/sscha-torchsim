...
#         return df_dfc, err_df_dfc
    ...
    def get_energy_forces(self, ase_calculator, compute_stress = True, stress_numerical = False, skip_computed = False, verbose = False, timer=None):
        ...
        results = CC.calculators.get_results_batch(ase_calculator, structures[start:stop], get_stress = compute_stress)
        _energies = results["energy"] / Rydberg # eV => Ry
        _forces = results["forces"].reshape((num_confs * nat3)) / Rydberg
        if compute_stress:
            _stresses = -results["stress"].reshape((num_confs, 9)) * Bohr**3 / Rydberg

        for i in range(num_confs):
            energies[i0] = _energies[i]
            forces[nat3*i0 : nat3*i0 + nat3] = _forces[nat3*i : nat3*i + nat3]
            if compute_stress:
                stress[9*i0 : 9*i0 + 9] = _stresses[i]
            run = False
            i0 += 1
